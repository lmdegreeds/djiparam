...
- **Все параметры** — поиск, категории и постраничное чтение 800–950+ параметров с ленивой
  подгрузкой (рециклинг строк под low-RAM экран). Каталог грузится **под выбранную модель**
  (`assets/params_*.json`), так что имена, индексы, диапазоны и категории — той модели, что выбрана.
...
- **Модельная независимость**: индекс параметра у каждой модели свой (`gps_enable`: 53 у Neo 2 …
  1403 у Flip), поэтому параметры резолвятся **по имени** через каталог активной модели
  (`assets/quick_index.json` + `assets/params_*.json`, 14 моделей — из дампов в [`params/`](params/)
  и таблиц по кодовым именам в [`params/model_tables/`](params/model_tables/)).
  **Автоопределение модели по CRC таблицы прошивки** (DUML `0xE0`; count разрешает коллизии CRC,
  напр. Lito X1 vs Mini 5 Pro), fallback по кодовому имени борта, плюс ручной выбор в верхней панели.
